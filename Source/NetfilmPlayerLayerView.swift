 ...
 */
public enum NetfilmPlayerState {
    case notSetURL
    case readyToPlay
    case buffering
    case bufferFinished
...
/**
 ...
 - four2THREE:   4:3
 */
public enum NetfilmPlayerAspectRatio : Int {
    case `default`    = 0
    case sixteen2NINE
    case four2THREE
}
@MainActor
public protocol NetfilmPlayerLayerViewDelegate : AnyObject {
    func NetfilmPlayer(player: NetfilmPlayerLayerView, playerStateDidChange state: NetfilmPlayerState)
    func NetfilmPlayer(player: NetfilmPlayerLayerView, loadedTimeDidChange loadedDuration: TimeInterval, totalDuration: TimeInterval)
    func NetfilmPlayer(player: NetfilmPlayerLayerView, playTimeDidChange currentTime: TimeInterval, totalTime: TimeInterval)
    func NetfilmPlayer(player: NetfilmPlayerLayerView, playerIsPlaying playing: Bool)
}



open class NetfilmPlayerLayerView: UIView {
    
    open weak var delegate: NetfilmPlayerLayerViewDelegate?
    ...
    open var isPlaying: Bool = false {
        didSet {
            if oldValue != isPlaying {
                delegate?.NetfilmPlayer(player: self, playerIsPlaying: isPlaying)
            }
        }
    }
    
    var aspectRatio: NetfilmPlayerAspectRatio = .default {
        didSet {
            self.setNeedsLayout()
        }
    }
    ...
    /// 播放器的几种状态
    fileprivate var state = NetfilmPlayerState.notSetURL {
        didSet {
            if state != oldValue {
    ...
    open func seek(to seconds: TimeInterval, completion: (@Sendable () -> Void)?) {
        if seconds.isNaN {
            return
        }
        setupTimer()
        ...
        if self.player?.currentItem?.status == AVPlayerItem.Status.readyToPlay {
            ...
            self.player!.seek(to: draggedTime, toleranceBefore: .zero, toleranceAfter: .zero) { [weak self] finished in
                guard let self = self else { return }
                
                Task { @MainActor in
                    // التأكد من استدعاء completion على الـ main thread
                    completion?()
                }
            }
        } else {
    ...
    @objc fileprivate func moviePlayDidEnd() {
        if state != .playedToTheEnd {
            if let playerItem = playerItem {
                delegate?.NetfilmPlayer(player: self,
            ...
            if item == self.playerItem {
                    ...
                    case "status":
                        if item.status == .failed || player?.status == .failed {
                            ...
                            self.state = .buffering
                            if shouldSeekTo != 0 {
                                print("NetfilmPlayerLayer | Should seek to \(shouldSeekTo)")
                    ...
                    case "loadedTimeRanges":
                        if let timeInterval = self.availableDuration() {
                            ...
                            delegate?.NetfilmPlayer(player: self, loadedTimeDidChange: timeInterval, totalDuration: totalDuration)
                        }
        ...
        DispatchQueue.main.asyncAfter(deadline: popTime) {[weak self] in
            ...
            if let item = self.playerItem {
                if !item.isPlaybackLikelyToKeepUp {
                    ...
                    self.state = NetfilmPlayerState.bufferFinished
                }
            }
        }
    }
